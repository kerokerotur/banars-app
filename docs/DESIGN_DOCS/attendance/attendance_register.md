...
sequenceDiagram
    ...
    alt 締切後
        ...
        Member->>App: コメント入力（常時表示のテキストフィールド、任意）
        Member->>App: 「回答する」ボタンをタップ

        App->>App: クライアント側バリデーション
        App->>Edge: POST /attendance_register
        Note right of App: eventId, status, comment
...
3. **締切前の場合（7-19）**:
   - **ステータス選択（7）**: 3つのボタン（出席/欠席/保留）のいずれかをタップ
   - **コメント入力（8）**: 常時表示のテキストフィールドに任意入力（文字数上限なし）
   - **送信操作（9）**: 「回答する」ボタンをタップしたタイミングで API 呼び出し
   - **クライアント側バリデーション（10）**: 必須項目のチェック（status のみ必須）
   - **認証（12-14）**: JWT トークン検証とイベント情報取得
   - **サーバー側締切チェック（15-17）**: 二重チェックとして締切を再確認（クライアント時計のずれ対策）
   - **データ登録（18-20）**: `(event_id, member_id)` のユニーク制約により UPSERT 実行
   - **UI更新（21）**: Edge Function のレスポンスをそのまま適用し、トースト通知で完了を知らせる

4. **コメント入力**: イベント詳細画面で常時表示。出席/欠席/保留すべてで任意入力、文字数上限なし。
...
| `comment` | `string` |  | 任意コメント（理由や補足情報、常時表示テキストフィールド。上限なし） |
...
| コメント長さ | 上限なし。空でも可（禁止バイトなし） | Client |
...
| エラー状況 | ユーザーへの導線 |
| --- | --- |
| バリデーションエラー | 該当フィールドにエラーメッセージを表示し修正を促す（例: ステータス未選択時「回答ステータスを選択してください」） |
