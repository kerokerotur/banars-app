...
sequenceDiagram
    ...
    alt 既存会場が見つかる
        ...
        App->>App: WebView で Google Maps URL をプレビュー表示
        App-->>User: 会場名入力フィールドと地図プレビューを表示
        User->>App: 地図を確認しながらイベント会場名を入力
        User->>App: 登録ボタン押下
...
1. **Google Maps 共有 URL** (必須)
   - テキスト入力（URL形式）
   - プレースホルダー: https://maps.app.goo.gl/xxxxx
   - ヘルプテキスト: 「Google Maps で場所を開き、共有→リンクをコピー で取得できます」
   - URL 入力後、自動的に `place_lookup` を実行
   - 既存会場が見つかった場合は既存会場情報を表示し、未登録の場合は次のステップへ

2. **地図プレビュー** (新規登録時のみ表示)
   - WebView で Google Maps URL を表示
   - ユーザーが入力した URL の内容を視覚的に確認できる
   - 会場名入力時の参考情報として活用

3. **イベント会場名** (必須、新規登録時のみ表示)
   - テキスト入力
   - プレースホルダー: 例）○○体育館
   - 最大文字数: 120文字
   - 地図プレビューを見ながら入力可能
...
イベント会場の登録・表示時に、WebView で `google_maps_url` を読み込むことで地図を表示する。

### 使用場面

1. **新規登録時のプレビュー**: URL 入力後、未登録の場合に地図プレビューを表示し、ユーザーが正しい場所を入力したか確認できるようにする
2. **会場情報表示時**: 登録済み会場の詳細を表示する際に、地図を表示する
...
- **新規登録時**: 地図プレビューと会場名入力フィールドを同時に表示し、ユーザーが地図を見ながら適切な会場名を入力できるようにする
- **プレビューの配置**: 会場名入力フィールドの上部に配置し、視線移動を最小限にする
- **プレビューのサイズ**: 画面幅の 80-100% 程度、高さは 400px を推奨
- **モーダルのサイズ**: 画面の最大90%の高さを使用し、十分な表示領域を確保

### 注意事項

- WebView は Android/iOS でのみ動作（Web版では別途対応が必要）
- インターネット接続が必要
- 地図の読み込みに時間がかかる場合があるため、ローディングインジケーターを表示することを推奨
