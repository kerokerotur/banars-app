...
ドキュメント種別:
- この CLAUDE.md は AI 向けの運用ルール集。README.md は人間読者向けなので、案内や解説は README.md に記載すること。
...
このプロジェクトはモノレポ構成のため、各ディレクトリごとに実装ルールと運用方針がドキュメント化されています。作業を行う際は、必ず該当するディレクトリの README.md を参照してください。

| 作業対象 | 参照先 | 主な内容 |
| --- | --- | --- |
| **Flutterアプリ開発** | `apps/mobile/README.md` | FVM の使用、derry スクリプト、MVVM + Riverpod アーキテクチャ、環境変数設定 |
| **バックエンド・ドメイン層** | `backend/README.md` | コンテキスト構造、リポジトリパターン、DI、Hono フレームワーク、テストルール、パスエイリアス |
| **Supabase（マイグレーション、Edge Functions）** | `infra/supabase/README.md` | マイグレーション作成・命名規則、Edge Functions 作成・バンドル・デプロイ、依存関係管理、シードデータ |
| **設計ドキュメント・データモデル** | `docs/DESIGN_DOCS/README.md` | 機能一覧とコンテキスト、データモデル運用ルール（外部キー制約の方針含む）、テーブル定義の管理方法 |
...
- 今後、ドキュメント運用や修正方針に関する新たな決定が発生した場合は、例外なく本 CLAUDE.md に追記して記録し、次回以降のチャットセッションでも同じ観点が参照できるようにする。
