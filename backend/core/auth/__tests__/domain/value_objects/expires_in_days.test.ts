...
describe("ExpiresInDays", () => {
  describe("fromRaw", () => {
    describe("正常系", () => {
      it("値を指定しない場合、デフォルト値（7）が設定される", () => {
        const expiresInDays = ExpiresInDays.fromRaw()
        expect(expiresInDays.toNumber()).toBe(7)
      })

      it("最小値（1）を指定できる", () => {
        const expiresInDays = ExpiresInDays.fromRaw(1)
        expect(expiresInDays.toNumber()).toBe(1)
      })

      it("最大値（30）を指定できる", () => {
        const expiresInDays = ExpiresInDays.fromRaw(30)
        expect(expiresInDays.toNumber()).toBe(30)
      })

      it("範囲内の値（15）を指定できる", () => {
        const expiresInDays = ExpiresInDays.fromRaw(15)
    ...
    describe("異常系", () => {
      it("0を指定するとエラーになる", () => {
        expect(() => ExpiresInDays.fromRaw(0)).toThrow(InviteIssueError)
        expect(() => ExpiresInDays.fromRaw(0)).toThrow(
          "expiresInDays は 1〜30 の範囲で指定してください。",
        )
      })

      it("31を指定するとエラーになる", () => {
        expect(() => ExpiresInDays.fromRaw(31)).toThrow(InviteIssueError)
        expect(() => ExpiresInDays.fromRaw(31)).toThrow(
          "expiresInDays は 1〜30 の範囲で指定してください。",
        )
      })

      it("負の値を指定するとエラーになる", () => {
        expect(() => ExpiresInDays.fromRaw(-1)).toThrow(InviteIssueError)
        expect(() => ExpiresInDays.fromRaw(-1)).toThrow(
          "expiresInDays は 1〜30 の範囲で指定してください。",
        )
      })

      it("小数を指定するとエラーになる", () => {
        expect(() => ExpiresInDays.fromRaw(7.5)).toThrow(InviteIssueError)
        expect(() => ExpiresInDays.fromRaw(7.5)).toThrow(
          "expiresInDays は整数で指定してください。",
        )
      })

      it("NaNを指定するとエラーになる", () => {
        expect(() => ExpiresInDays.fromRaw(NaN)).toThrow(InviteIssueError)
        expect(() => ExpiresInDays.fromRaw(NaN)).toThrow(
          "expiresInDays は整数で指定してください。",
        )
      })
    })
  ...
  describe("calculateExpiresAt", () => {
    it("基準日から指定日数後の日付を返す", () => {
      const expiresInDays = ExpiresInDays.fromRaw(7)
